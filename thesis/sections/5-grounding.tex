...
\cmtDK[inline]{general setup: epochs, optimizer, batch size, ...}
...
The experiments are conducted with the following hyperparameters: a learning rate of $2\times10^{-4}$, a batch size of 32 samples and 30 epochs, Adam \citep{Kingma2015} as optimizer.
...
In the first setup, the model receives the bounding boxes as input.
...
The first model acts as the baseline and only receives the complete image as the input.
...
Therefore, it is extended in a second step with attention to the target object.
...
For both setups, the same hyperparameters as in the previous experiments are used: a learning rate of $2\times10^{-4}$, a batch size of 32 samples and 30 epochs, Adam \citep{Kingma2015} as optimizer.
8000 randomly selected samples are used for training, the remaining 2000 samples for testing.
The loss is calculated using cross entropy.
...
For all setups, the same hyperparameters as in the previous experiments are used: a learning rate of $2\times10^{-4}$, a batch size of 32 samples and 30 epochs, Adam \citep{Kingma2015} as optimizer.
8000 randomly selected samples are used for training, the remaining 2000 samples for testing.
The loss is calculated using cross entropy.
The following values for the variables are compared:
...
\begin{table}[ht]
    ...
    \begin{tabular}{lcccc}
        ...
        Coordinate predictor   & \times             & \times            & -         & -                      \\
        + one-hot              & \times             & \times            & -         & -                      \\
