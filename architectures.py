...
class Receiver(nn.Module):
    def __init__(self, n_hidden, n_labels, encoded_image_size=1) -> None:
        ...
        self.resnet = nn.Sequential(*list(resnet.children())[:-2])
        self.adaptive_pool = nn.AdaptiveAvgPool2d((encoded_image_size, encoded_image_size))
        self.linear = nn.Linear(2048, n_hidden)
    ...
    def forward(self, x, _input, _aux_input):
        ...
        for obj in _input:
            ...
            reduced = self.relu(self.linear(pooled.squeeze()))
            reduced = reduced.unsqueeze(dim=1)
            multiplied = torch.matmul(reduced, x)
            after_resnet.append(multiplied.squeeze())

        stacked = torch.stack(after_resnet, dim=1)
        # permute back
